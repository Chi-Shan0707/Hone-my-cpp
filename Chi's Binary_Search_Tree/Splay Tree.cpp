...
//closewise顺时针
  ...
  //每一步操作都要注意，双向关系的维护   
    //先搞定原爷爷
    if(tr[tr[parent].pa].child[LEFT]==parent)tr[tr[parent].pa].child[LEFT]=id;
    else tr[tr[parent].pa].child[RIGHT]=id;
    tr[id].pa=tr[parent].pa;

    //再搞定自己的儿子
    
    tr[parent].child[LEFT]=tr[id].child[RIGHT];
    tr[tr[id].child[RIGHT]].pa=parent;
